...
public class DASetConfig extends NodeConfig implements NodeConfigBase
{
    ...
    /** 物联参数数据：是否按偏移量排序。默认为：false，按用户指定的报文集合顺序 */
    private NodeParam           callParamOffsetSort;
    
    /** 物联参数数据 */
    private String              dataXID;
    
    /** 物联参数数据的实例对象(内部使用) */
    private Map<String ,Object> dataXIDObject;
    ...
    public DASetConfig(long i_RequestTotal ,long i_SuccessTotal)
    {
        ...
        this.callParamOffsetSort = new NodeParam();
    ...
    {
        this.dataXID       = i_DataXID;
        this.dataXIDObject = null;
        this.reset(this.getRequestTotal() ,this.getSuccessTotal());
        this.keyChange();
    }
    ...
    private synchronized Map<String ,Object> gatDataXIDObject() throws Exception
    {
        if ( this.dataXID == null || this.dataXID == null )
        {
            return null;
        }
        
        if ( this.dataXIDObject == null )
        {
            XJSON v_XJson = new XJSON();
            this.dataXIDObject = (Map<String ,Object>) v_XJson.toJava(this.dataXID ,Map.class);
        }
        
        return this.dataXIDObject;
    }
    
    
    
    /**
     * 获取：是否按偏移量排序。默认为：false，按用户指定的报文集合顺序
     */
    public Boolean getOffsetSort()
    {
        return Boolean.valueOf(this.callParamOffsetSort.getValue());
    }

    
    
    /**
     * 设置：是否按偏移量排序。默认为：false，按用户指定的报文集合顺序
     * 
     * @param i_OffsetSort 是否按偏移量排序。默认为：false，按用户指定的报文集合顺序
     */
    public void setOffsetSort(Boolean i_OffsetSort)
    {
        this.callParamOffsetSort.setValue(i_OffsetSort == null ? "false" : i_OffsetSort.toString());
        this.reset(this.getRequestTotal() ,this.getSuccessTotal());
        this.keyChange();
    }
    ...
    public Object [] generateParams(Map<String ,Object> io_Context ,Object [] io_Params)
    {
        try
        {
            List<MDataItem>     v_Datagram    = (List<MDataItem>) io_Params[1];
            List<MDataItem>     v_DatagramNew = new ArrayList<MDataItem>();
            Map<String ,Object> v_DataXID     = this.gatDataXIDObject();
            
            for (MDataItem v_Item :v_Datagram)
            {
                MDataItem v_ItemNew = new MDataItem(v_Item.getName() ,v_Item.getOffset() ,v_Item.getDataType());
                Object    v_Data    = ValueHelp.getValue(ValueHelp.standardRefID(v_Item.getName()) ,null ,v_DataXID ,io_Context);
                if ( v_Data == null )
                {
                    v_ItemNew.setData("");
                }
                ...
                {
                    v_ItemNew.setData(v_Data.toString());
                }
                v_DatagramNew.add(v_ItemNew);
            }
            
            io_Params[1] = v_DatagramNew;
            return io_Params;
        }
